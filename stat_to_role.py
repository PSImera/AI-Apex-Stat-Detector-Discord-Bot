...
async def role_by_stats(**kwargs):
    base_stats_miss = [
        k
        for k in ["kills", "deaths", "kd", "counted_kd", "avg", "is_ranked"]
        if k not in kwargs
    ]
    rank_stats_miss = [
        k
    ...
    if base_stats_miss or (rank_stats_miss and season_stats_miss):
        ...
        df["kd"] = kd_check(
            kwargs.get("kd"), kwargs.get("counted_kd"), kwargs.get("deaths")
        )
        if is_ranked:
            df["rank_kd"] = kd_check(
                kwargs.get("rank_kd"),
                kwargs.get("rank_counted_kd"),
                kwargs.get("rank_deaths"),
            )
            rank_map = {
                "unknown": 0,
                "bronze": 1,
            ...
            df["season_kd"] = kd_check(
                kwargs.get("season_kd"),
                kwargs.get("season_counted_kd"),
                kwargs.get("season_deaths"),
            )
            rank = "0"
