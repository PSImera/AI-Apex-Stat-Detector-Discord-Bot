...
async def set_channel(interaction: discord.Interaction, channel: discord.TextChannel):
    if interaction.user.guild_permissions.administrator:
        ...
        await interaction.response.send_message(
            f"Channel {channel.mention} has been successfully configured",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
...
async def set_log_channel(
    ...
    if interaction.user.guild_permissions.administrator:
        ...
        save_config(config)
        await interaction.response.send_message(
            f"Log channel {channel.mention} has been successfully configured",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
...
@tree.command(
    name="set_skill_roles", description="skill roles. From [1] weak to [6] strong"
)
@discord.app_commands.describe(index="Skill level", role="role on server")
async def set_skill_roles(
    interaction: discord.Interaction, index: str, role: discord.Role
):
    if interaction.user.guild_permissions.administrator:
        guild_id = str(interaction.guild_id)
        if guild_id not in config:
        ...
        save_config(config)
        await interaction.response.send_message(
            f"Role {role.name} for {index} has been successfully configured",
            ephemeral=True,
        )
    else:
        await interaction.response.send_message(
...
@tree.command(
    name="set_rank_roles",
    description="rank roles. [0] not ranked & From [1] bronze to [7] predator",
)
...
async def set_mode(
    ...
    if interaction.user.guild_permissions.administrator:
        ...
        save_config(config)
        await interaction.response.send_message(
            f"Bot is now in '{mode.name}' mode.", ephemeral=True
        )
    else:
        await interaction.response.send_message(
            "You do not have sufficient permissions to execute this command.",
...
async def on_message(message: discord.Message):
    ...
    if message.attachments:
        ...
        print(
            f"deleted spam in {message.channel.name} channel by {message.author.name}"
        )
        return
    ...
    if len(message.attachments) == 1:
        ...
        await message.channel.send(
            f"{message.author.mention} [error] send only 1 attachment with your statistic screen please"
        )
        return
    ...
    if attachment.filename.endswith((".png", ".jpg", ".jpeg", ".gif", ".bmp")):
        ...
        await message.channel.send(
            f"{message.author.mention} [error] send only image files (png, jpg, jpeg, gif, bmp) with your statistic screen please"
        )
        return
    ...
    if image.size[0] >= 1280 and image.size[1] >= 720:
        ...
        await message.channel.send(
            f"{message.author.mention} [error] send image more then 1280x720"
        )
        return
    ...
    if all(
        ...
        current_skill_roles = [
            rl_name
            for rl_name, rl_id in roles_map_skill.items()
            if discord.utils.get(message.guild.roles, id=rl_id) in message.author.roles
        ]
        current_rank_roles = [
            rl_name
            for rl_name, rl_id in roles_map_rank.items()
            if discord.utils.get(message.guild.roles, id=rl_id) in message.author.roles
        ]
        for cr_name in current_skill_roles:
            await message.author.remove_roles(
                discord.utils.get(message.guild.roles, id=roles_map_skill.get(cr_name))
            )
        ...
        if skill == "error":
            await message.channel.send(
                f"{message.author.mention} [error] cant reed stats from this image. try make better screen"
            )
            await log_channel.send(log, file=dc_file)
            print(f"[BOT] error, {message.author.name} not get role, cant read stats")
            return

        if mode == "both_roles":
            skill_role = discord.utils.get(
                message.guild.roles, id=roles_map_skill.get(skill)
            )
            rank_role = discord.utils.get(
                message.guild.roles, id=roles_map_rank.get(rank)
            )
            ...
            await message.channel.send(
                f"{message.author.mention} you get roles **{skill_role}** and **{rank_role}**"
            )
            print(
                f"[BOT] {message.author.name} get roles skill: {skill} & rank: {rank}"
            )
            await log_channel.send(
                f"{message.author.mention}{skill_role.mention}{rank_role.mention}"
                + log,
                file=dc_file,
            )
            ...
            await message.channel.send(
                f"{message.author.mention} you get role **{skill_role}**"
            )
            print(f"[BOT] {message.author.name} get skill role {skill}")
            await log_channel.send(
                f"{message.author.mention}{skill_role.mention}" + log,
                file=dc_file,
            )
        elif mode == "rank_only":
            rank_role = discord.utils.get(
            ...
            await message.channel.send(
                f"{message.author.mention} you get role **{rank_role}**"
            )
            print(f"[BOT] {message.author.name} get rank role {rank}")
            await log_channel.send(
                f"{message.author.mention}{rank_role.mention}" + log,
                file=dc_file,
            )
        else:
